  ...
  try {
    ...
    for (const group of tabGroups) {
      ...
      if (groupName && groupName.length <= 4 && /^[a-z]+$/.test(groupName)) {
        debugLog(
          "[ATO Background] Found potentially generic short group:",
          group.title,
        );
        return true;
      }
  ...
  try {
    ...
    autoOrganizeTimeout = setTimeout(async () => {
      try {
        ...
        const autoModeRecategorizeGrouped =
          settings.autoModeRecategorizeGrouped || "smart";

        switch (autoModeRecategorizeGrouped) {
          case "always":
            includeGrouped = true;
            debugLog(
              "[ATO Background] Auto Mode: Always recategorizing grouped tabs",
            );
            break;
          case "never":
            includeGrouped = false;
            debugLog(
              "[ATO Background] Auto Mode: Never recategorizing grouped tabs",
            );
            break;
          case "smart":
          default:
            ...
            debugLog(
              "[ATO Background] Auto Mode: Smart detection -",
              includeGrouped ? "including" : "excluding",
              "grouped tabs",
            );
            break;
        }
